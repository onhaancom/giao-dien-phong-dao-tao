...
try {
    ...
    if ($stmt->execute()) {
        // Get the updated class with course info
        $getQuery = "SELECT c.*, co.course_name, co.course_code 
                    FROM classes c
                    LEFT JOIN courses co ON c.course_id = co.id
                    WHERE c.id = :id";
        $getStmt = $db->prepare($getQuery);
        $getStmt->bindParam(':id', $class_id);
        $getStmt->execute();
        $updatedClass = $getStmt->fetch(PDO::FETCH_ASSOC);
        ...
        throw new Exception("Failed to update class: " . $errorInfo[2]);
    }
