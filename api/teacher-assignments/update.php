...
if (empty($data->assignment_id) || empty($data->teacher_id) || empty($data->class_id) || empty($data->assignment_type)) {
    $response['message'] = 'Assignment ID, Teacher ID, Class ID, and Assignment Type are required';
...
try {
    ...
    if ($stmt->execute()) {
        ...
        $response['message'] = 'Failed to update teacher assignment';
