...
try {
    ...
    $log['input'] = ['curriculum_id' => $curriculum_id];
    ...
    $checkQuery = "SELECT id, title FROM curriculum WHERE id = :id";
    ...
    if (!$item) {
        throw new Exception("Curriculum item not found with ID: $curriculum_id");
    }
    ...
    $deleteStmt->bindParam(':id', $curriculum_id, PDO::PARAM_INT);
    
    $log['delete_attempt'] = [
        'query' => $deleteQuery,
        'params' => ['id' => $curriculum_id]
    ];
    ...
    $response = [
        'status' => 'success',
        'message' => 'Curriculum item "' . $item['title'] . '" deleted successfully',
...
file_put_contents('curriculum_delete.log', date('Y-m-d H:i:s') . " - " . json_encode($log) . "\n", FILE_APPEND);
?>